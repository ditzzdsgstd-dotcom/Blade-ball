...
function createArrowGui()
	...
	local up = createButton("Up", UDim2.new(centerX, 0, centerY - 0.1, 0), "â†‘")
	local down = createButton("Down", UDim2.new(centerX, 0, centerY + 0.1, 0), "â†“")
	local left = createButton("Left", UDim2.new(centerX - 0.1, 0, centerY, 0), "â†")
	local right = createButton("Right", UDim2.new(centerX + 0.1, 0, centerY, 0), "â†’")
...
local StatModule = MicTab:create_module({
    ...
    callback = function(value)
        if value then
			...
			frame.BorderSizePixel = 0
			frame.Active = true
			frame.Draggable = true -- ðŸŸ¢ Cho phÃ©p kÃ©o
...
local VisualizeModule = MicTab:create_module({
    ...
    callback = function(value)
        if value then
			-- Táº¡o forcefield visual náº¿u chÆ°a cÃ³
			...
			visualizeConnection = RunService.RenderStepped:Connect(function()
				...
				-- TÃ­nh khoáº£ng cÃ¡ch co giÃ£n (clamp tá»« 25 Ä‘áº¿n 400)
